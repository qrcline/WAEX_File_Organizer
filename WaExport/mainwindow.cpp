...
void MainWindow::on_openFolder_clicked()
{
   ...
   if (fIo.checkForDirect(ui->POInput->text())) //If the main directory is selected check the PO# that is input
    {
...
void MainWindow::on_mexP_Spreadsheets_upload_clicked()
{
    ...
    if (QFile::exists(mainDirectory+"/Spreadsheets.xlsx"))
    {
        QMessageBox::information(0,"Error","Error:This file already loaded.");
        return;
    }
    //copy Syntax
    //copy(from,to)
     FolderIO fIo;
    fIo.copyFile(filePath,mainDirectory,"Spreadsheets.xlsx");
